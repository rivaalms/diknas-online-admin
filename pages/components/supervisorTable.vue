...
export default {
   ...
   methods: {
      ...
      async dialog(type, item) {
         ...
         if (type === 'new') {
            this.dialogTitle = 'Input Pengawas'
            this.targetItem = {
               name: null,
               nip: null,
               email: null,
            ...
            this.isEdit = true
            this.dialogTitle = 'Edit Pengawas'
            this.targetItem = {
               id: item.id,
               name: item.name,
               nip: item.nip,
         ...
         if (this.$refs.form.validate()) {
            if (type === 'new') {
               await this.$axios.post(`/supervisor/create`, this.targetItem).then((resp) => {
                  ...
                  this.alertMessage = `Pengawas baru berhasil dibuat. ID pengawas: <code>${resp.data.data.id}</code>`
               ...
               await this.$axios.put(`/supervisor/update/${this.targetItem.id}`, this.targetItem).then(resp => {
                  ...
                  this.alertType = 'info'
                  this.alertMessage = `Data pengawas <code>${resp.data.data.id}</code> berhasil disunting`
               }).catch(e => {console.log(e)})
            }
      ...
      async confirmDelete(item) {
         await this.$axios.delete(`/supervisor/delete/${item.id}`).then((resp) => {
            this.alertColor = 'amber darken-2'
            this.alertIcon = 'mdi-information'
            this.alertType = 'warning'
            this.alertMessage = `Data pengawas <code>${item.id}</code> berhasil dihapus`
            document.getElementById('alertMessage').innerHTML = this.alertMessage
            this.alertTrigger = true
