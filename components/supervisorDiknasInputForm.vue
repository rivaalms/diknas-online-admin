...
<v-card-text>
   <v-form ref="form" aria-autocomplete="off" @submit.prevent="submit(dialog.type)">
      <v-row>
         ...
         <template
            v-if="dialog.type === 'supervisor-edit' || dialog.type === 'diknas-edit'"
         >
            <v-col cols="12">
               <v-checkbox
...
export default {
   ...
   beforeMount() {
      if (this.dialog.type === 'supervisor-edit' || this.dialog.type === 'diknas-edit') {
         this.targetItem = {
      ...
      } else if (this.dialog.type === 'supervisor-new' || this.dialog.type === 'diknas-new') {
         this.targetItem = {
            name: null,
            nip: null,
            email: null,
      ...
      async submit(type) {
         if (this.$refs.form.validate()) {
            if (type === 'supervisor-edit') {
               await this.$axios.put(`/supervisor/update/${this.targetItem.id}`, this.targetItem).then((resp) => {
                  this.$store.dispatch('setAlert', {
                     type: 'info',
                     color: 'cyan darken-2',
                     icon: 'mdi-information',
                     message: `Data pengawas <code>${resp.data.data.id}</code> berhasil disunting`
                  })
               }).catch((e) => {
                  this.$store.dispatch('setAlert', {
                     type: 'error',
               ...
               await this.$axios.post(`/supervisor/create`, this.targetItem).then((resp) => {
                  this.$store.dispatch('setAlert', {
                     type: 'success',
                     color: 'green darken-2',
                     icon: 'mdi-check-circle',
                     message: `Pengawas baru berhasil dibuat. ID pengawas: <code>${resp.data.data.id}</code>`
                  })
               }).catch((e) => {
                  this.$store.dispatch('setAlert', {
                     type: 'error',
            ...
            } else if (type === 'diknas-edit') {
               await this.$axios.put(`/diknas/update/${this.targetItem.id}`).then((resp) => {
                  this.$store.dispatch('setAlert', {
                     type: 'info',
                     color: 'cyan darken-2',
                     icon: 'mdi-information',
                     message: `Data diknas <code>${resp.data.data.id}</code> berhasil disunting`
                  })
               }).catch((e) => {
                  this.$store.dispatch('setAlert', {
                     type: 'error',
